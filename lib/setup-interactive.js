...
async function runInteractiveAuth(hostname) {
  ...
  console.log('  This will open a browser URL for you to authorize this device.');
  ...
  return new Promise((resolve, reject) => {
    ...
    tailscale.stdout.on('data', (data) => {
      ...
      stdout += str;
      const match = str.match(/(https:\/\/login\.tailscale\.com\/[^\s]+)/);
      if (match && !authURL) {
        authURL = match[1];
        console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        console.log('');
        ...
        console.log('  ' + authURL);
        console.log('');
        console.log('  Or run: tailscale up --login-server ' + authURL);
        console.log('');
        console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        console.log('');
      }
    ...
    tailscale.on('close', (code) => {
      if (code === 0) {
        ...
        // Check if it's just waiting for auth
        if (stdout.includes('login.tailscale.com') || stderr.includes('login.tailscale.com')) {
          console.log('  ⏳ Waiting for authorization...');
          console.log('  Press Enter once you\'ve authorized this device in your browser.');
          resolve('waiting');
        } else {
          reject(new Error(`tailscale up failed: ${stderr || stdout}`));
        }
      }
    });
    
    tailscale.on('error', reject);
  });
}
...
async function runSetup() {
  ...
  // Step 4: Handle interactive auth or save auth key
  if (useInteractive) {
    console.log('\nConfiguring interactive authentication...');
    const result = await runInteractiveAuth(hostname);
    ...
    if (result === true) {
      ...
      const status = waitForTailscale(10);
      if (status) {
        console.log('✓ Interactive authentication successful');
      } else {
    ...
    // For interactive auth, we don't save an auth key
    // Tailscale stores state in /var/lib/tailscale/
    console.log('✓ Interactive auth configured (state persisted in /var/lib/tailscale/)');
  } else {
    // Save auth key
    console.log('\nConfiguring...');
